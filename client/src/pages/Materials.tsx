...
export default function Materials() {
  ...
  // Auto-expand cards when searching with smooth animation
  useEffect(() => {
    if (searchQuery) {
      setIsAnimating(true);
  ...
  const scrollToType = (type: string) => {
    ...
    if (typeElement) {
      ...
      // Calculate the element's position with smooth animation
      const elementRect = typeElement.getBoundingClientRect();
      const elementPosition = window.pageYOffset + elementRect.top;
      
      // Calculate the final scroll position with additional padding for smoother appearance
      const scrollPosition = elementPosition - navbarHeight - 40; // Increased padding for smoother look
  ...
  const handleSidebarClick = (type: string) => {
    ...
    // Enhanced smooth animation timing
  ...
  const handleCardClick = (type: string) => {
    ...
    setCardExpandedTypes(prev => {
      ...
      // Enhanced smooth animation for expansion
  ...
  return (
    <div className={`materials-page ${isAnimating ? 'animating' : ''}`}>
      <ScrollToTop />
      ...
      <div className="materials-layout">
        ...
        <aside className={`materials-sidebar ${isAnimating ? 'animating' : ''}`}>
